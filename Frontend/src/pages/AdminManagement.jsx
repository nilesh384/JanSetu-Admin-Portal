...
import { getAllAdmins } from "../api/user";
...
export default function AdminManagement() {
  ...
  const filteredAndSortedAdmins = () => {
    let filtered = admins.filter(admin => {
      ...
      const matchesDepartment = departmentFilter === "all" || admin.department?.toLowerCase() === departmentFilter.toLowerCase();
  ...
  const superAdmins = filteredAdmins.filter(admin => admin.role?.toLowerCase() === 'super_admin').length;
  const regularAdmins = filteredAdmins.filter(admin => admin.role?.toLowerCase() === 'admin').length;
  const viewers = filteredAdmins.filter(admin => admin.role?.toLowerCase() === 'viewer').length;
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-4">
              ...
              <div className="bg-white rounded-xl px-4 py-3 shadow-sm border border-emerald-200">
                <div className="text-xs text-emerald-600 uppercase tracking-wide">Active</div>
                <div className="text-2xl font-bold text-emerald-600">{activeAdmins}</div>
              </div>
              <div className="bg-white rounded-xl px-4 py-3 shadow-sm border border-gray-200">
                <div className="text-xs text-gray-600 uppercase tracking-wide">Super Admins</div>
                <div className="text-2xl font-bold text-gray-600">{superAdmins}</div>
              </div>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredAdmins.map((admin, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-full border ${getRoleColor(admin.role)}`}>
                        <span className={`w-2 h-2 rounded-full ${
                          admin.role?.toLowerCase() === 'super_admin' ? 'bg-gray-600' :
                          admin.role?.toLowerCase() === 'admin' ? 'bg-slate-600' :
                          'bg-emerald-600'
                        }`}></span>
                        {admin.role?.charAt(0).toUpperCase() + admin.role?.slice(1) || 'Viewer'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-900 capitalize">{admin.department || 'N/A'}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-600">{formatDate(admin.lastLogin)}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2 py-1 text-xs font-medium rounded-full ${
                        admin.lastLogin ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
                      }`}>
                        <span className={`w-2 h-2 rounded-full ${
          ...
          {filteredAdmins.length === 0 && (
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {admins.length === 0 ? 'No Admins Found' : 'No Results Match Your Search'}
              </h3>
          ...
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <div className="bg-gradient-to-r from-slate-700 to-slate-900 px-6 py-4 rounded-t-md">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      <h3 className="text-xl font-bold text-white">{selectedAdmin.fullName}</h3>
              ...
              <div className="px-6 py-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded">{selectedAdmin.fullName}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Email Address</label>
                        <p className="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded">{selectedAdmin.email}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Department</label>
                        <p className="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded">{selectedAdmin.department}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Role</label>
                        <span className={`inline-flex px-3 py-1 text-sm font-semibold rounded-full border ${getRoleColor(selectedAdmin.role)}`}>
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded font-mono">{selectedAdmin.id}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded">{formatDate(selectedAdmin.lastLogin)}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">Account Created</label>
                        <p className="mt-1 text-sm text-gray-900 bg-gray-50 px-3 py-2 rounded">{formatDate(selectedAdmin.createdAt)}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8">
                  <h4 className="text-lg font-semibold text-gray-900 mb-4">Media & Documents</h4>
                  <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Photos, videos, audio files, and documents will be displayed here when available.
                    </p>
                  </div>
                </div>
              </div>
